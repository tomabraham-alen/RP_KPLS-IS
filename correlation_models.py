...
def absolute_exponential(theta, d):  # pragma: no cover
    """
...
def squared_exponential(theta, d):  # pragma: no cover
    """
...
def generalized_exponential(theta, d):  # pragma: no cover
    """
...
def pure_nugget(theta, d):  # pragma: no cover
    """
...
def cubic(theta, d):  # pragma: no cover
    """
...
def linear(theta, d):  # pragma: no cover
    """
...
def matern32(theta, d):  # pragma: no cover
    ...
    if theta.size == 1:
        ...
        raise ValueError("Length of theta must be 1 or %i" % n_features)
    else:
        ll = theta.reshape(1, n_features) * np.abs(d)

    res = (1 + np.sqrt(3) * ll).prod(axis=1) * \
        np.exp(-np.sqrt(3) * (ll.sum(axis=1)))
    return res


def matern52(theta, d):  # pragma: no cover
    ...
    res = (1 + np.sqrt(5) * ll + 5. / 3 * ll**2).prod(axis=1) \
        * np.exp(-np.sqrt(5) * (ll.sum(axis=1)))
    return res
